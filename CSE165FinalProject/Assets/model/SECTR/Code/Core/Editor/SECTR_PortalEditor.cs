...
public class SECTR_PortalEditor : SECTR_HullEditor 
{
	...
	void _DrawViewportGUI(SECTR_Portal myPortal)
	{
		...
		else if(Selection.gameObjects.Length == 1)
		{
			float height = 100;
#if UNITY_5_4
			float editorScaleFactor = 1f / EditorGUIUtility.pixelsPerPoint;
#else
			float editorScaleFactor = 1f;
#endif
			GUILayout.BeginArea(new Rect((Screen.width * 0.5f * editorScaleFactor) - (width * 0.5f), Screen.height * editorScaleFactor  - height, width, height), "Where am I");
